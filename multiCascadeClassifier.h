...
class FaceDetection {
public:
    FaceDetection(const String& imagePath, const vector<String>& cascadePaths, const String& logFilePath);
...
FaceDetection::FaceDetection(const String& imagePath, const vector<String>& cascadePaths, const String& logFilePath)
    : logFilePath(logFilePath), totalUniqueFaces(0) {

    if (imagePath.empty() || cascadePaths.empty()) {
        cerr << "Error: Image path or cascade paths are empty." << endl;
        exit(EXIT_FAILURE);
    }
    ...
    for (const auto& cascadePath : cascadePaths) {
        ...
        if (!cascade.load(cascadePath)) {
            cerr << "Error: Could not load face cascade classifier: " << cascadePath << endl;
            exit(EXIT_FAILURE);
        }
...
void faceDetection(const String& imagePath, const vector<String>& cascadePaths, const String& logFilePath) {
    FaceDetection faceDetectionObj(imagePath, cascadePaths, logFilePath);
